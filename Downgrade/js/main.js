...
function parseOutput(value)
{
	...
	for (var i = 0, j = value.length; i < j; i++) {
		var element = document.createElement("tr");
		var appendTextt = "";
		element.innerHTML += ("<td><a style='color: #e6e6e6;' target='_blank' href='/BaseArchive/" + value[i][0] + "'>" + value[i][0] + "</a><br><span class=\"badge badge-info\">" + value[i][5] + "</span></td>"); // ROMID
		...
		if (!detectMob()) {
			buttons += " <button type='button' class='btn btn-info btn-sm sbtn' onclick='AdditionalOptions(this.parentElement.parentElement)'>...</button>";
		}
...
var targetRomID, targetRomLink;
function AdditionalOptions(element)
{
	var children = element.childNodes;
	var RomID = children[0].childNodes[0].innerText, RomLink;
	if (children[4].childNodes[0].tagName == "BUTTON")
	{
		RomLink = children[4].childNodes[1].href;
	}
	else
	{
		RomLink = children[4].childNodes[0].href;
	}
	
	targetRomID = RomID;
	targetRomLink = RomLink;
	$('#modalTitle').text(children[2].childNodes[0].innerText);
...
function CheckRomStatus(element)
{
	element.disabled = true;
	
	let targetID = targetRomID;
	$.ajax({
        type: "POST",
        cache: false,
        async: true,
        ...
        success: function (msg) {
			if (targetRomID == targetID)
			{
				if (msg == "YES")
				{
					element.className = "btn btn-teal";
					element.innerText = "Approved For Installation";
				}
				else
				{
					element.className = "btn btn-warning";
					element.innerText = "Not Approved";
				}
        ...
        error: function (msg) {
			if (targetRomID == targetID)
			{
				Notify("Couldn't check the rom. ( <span style='cursor: pointer; text-decoration: underline;' onclick=\"window.open('https://github.com/ProfessorJTJ/HISuite-Proxy/releases/')\">Get Latest HISuite Proxy</span> )", false);
				element.disabled = false;
			}
		}
    });
...
function GetRomFileList(element)
{
	element.disabled = true;
	
	let targetLink = targetRomLink;
	$.ajax({
        type: "POST",
        cache: false,
        async: true,
        ...
        data: targetLink,
        success: function (msg) {
			if (targetRomLink == targetLink)
			{				
				if (ProcessRomdata(element, msg) > 0)
				{
					element.remove();
				}
				else
...
function ProcessRomdata(element, romData)
{
	...
	let where = 0;
	while ((where = romData.indexOf("<vendorInfo", where)) != -1)
	{
		where = romData.indexOf("package=", where);
		if (where == -1)
			break;	
		where += 9;
		
		let finish = romData.indexOf('"', where);	
		let packageName = romData.substring(where, finish);
		
		let packagesubPath = "";
		
		where = romData.indexOf("subpath=", where);
		if (where != -1)
		{
			where += 9;
			
			finish = romData.indexOf('"', where);	
			packagesubPath = romData.substring(where, finish);
			if (packagesubPath.length > 2 && !packagesubPath.endsWith("/"))
			{
				packagesubPath += "/";
			}
		}
		
		if (!insertedItems.has((packagesubPath + packageName)))
		{
			insertedItems.set((packagesubPath + packageName), true);
			
			let newChild = document.createElement("a");
			newChild.target = "_blank";
			newChild.href = targetRomLink.replace("filelist.xml", packagesubPath + packageName);
			newChild.innerHTML = "<button class='btn btn-teal'>" + packageName + "</button>";
			//newChild.className = "btn btn-teal";
...
var xhr = $.ajax({
    ...
    success: function () {
       var LastModification = xhr.getResponseHeader('Content-Length'), LocalLastLoad = localStorage.getItem("LLDoad"), LocalLastLoadTime = localStorage.getItem("LLDoadT");
		if (LocalLastLoad == null || LocalLastLoadTime == null || LocalLastLoad != LastModification) {
			localStorage.setItem("LLDoad", LastModification);
